...
`metalsinglecell` promises it installs and imports anywhere — heavy backends (mlx, scanpy,
torch, squidpy) are lazy-imported *inside functions*, never at module load. This is what lets
`pip install metalsinglecell` succeed on Linux/Intel (no Metal) and in a minimal CI runner.
...
def test_package_import_pulls_no_heavy_backend():
    loaded = _import_and_report("import metalsinglecell")
    assert loaded == set(), f"import metalsinglecell eagerly loaded heavy backends: {loaded}"


def test_public_submodules_import_clean():
    loaded = _import_and_report(
        "import metalsinglecell as m; _ = (m.pp, m.tl, m.gr, m.config)")
    assert loaded == set(), f"pp/tl/gr/config import eagerly loaded: {loaded}"


def test_version_matches_pyproject():
    import metalsinglecell
    import tomllib
    from metalsinglecell import config
    with open(config.REPO_ROOT / "pyproject.toml", "rb") as f:
        pyproject_version = tomllib.load(f)["project"]["version"]
    assert metalsinglecell.__version__ == pyproject_version


def test_public_namespaces_present():
    import metalsinglecell as m
